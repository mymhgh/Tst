// গুগল শিট কনফিগারেশন (আপনার ডাটা দিয়ে রিপ্লেস করুন)
const GOOGLE_SHEET_ID = "15voIWEha4Er8wAsEf61IpgkAMQSSUayfE-CH1xC8aTs";
const GOOGLE_API_KEY = "AIzaSyAEioQJ6CuTPPH6YzcYOYuWT6GO-kboJQU";
const AUTHORIZED_RANGE = "Sheet1!A2:A100";
...
async function isUserAuthorized(chatId) {
    const url = `https://sheets.googleapis.com/v4/spreadsheets/${GOOGLE_SHEET_ID}/values/${AUTHORIZED_RANGE}?key=${GOOGLE_API_KEY}`;
    
    try {
        const response = await fetch(url);
        const data = await response.json();
        return data.values?.some(row => row[0] === chatId.toString()) || false;
    } catch (error) {
...
// ফর্ম সাবমিশন হ্যান্ডলার (আপডেটেড)
async function handleFormSubmission() {
    const chatId = document.getElementById("chatId").value;

    // ১. অথরাইজেশন চেক
    if (!await isUserAuthorized(chatId)) {
        showAlert('error', 'অনুমতি denied', 'আপনার কোন ফাইলই আমাদের কাছে নাই। আগে কাজ জমা দিন এরপর রিপোর্ট দেখে পেমেন্ট নিন!');
        return;
    }
